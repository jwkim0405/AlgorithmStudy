			...
			for (int j=2; j<N+2; j++) {
				...
				while(change<2) {
					if(arr[i][k]==arr[i][j]) {
						...
						count++;
						k++;
					} else if (arr[i-1][j-1]==arr[i][j]||arr[i+1][j-1]==arr[i][j]||arr[i+1][k]==arr[i][j]||arr[i-1][k]==arr[i][j]) {
						count++;
						break;
			...
			for (int j=2; j<N+2; j++) {
				...
				while(change<2) {
					if(arr[k][j]==arr[i][j]) {
						...
						count++;
						k++;
					} else if (arr[i-1][j-1]==arr[i][j]||arr[i-1][j+1]==arr[i][j]||arr[k][j+1]==arr[i][j]||arr[k][j-1]==arr[i][j]) {
						count++;
						break;
